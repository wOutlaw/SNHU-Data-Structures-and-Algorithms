...
// Description : Hash Tables and Chaining
//============================================================================
...
private:
	...
	struct Node {
		...
		// constructor that takes a bid
		Node(Bid aBid) : Node() {
			bid = aBid;
		}
		 
		// constructor that takes a bid and key
...
void HashTable::Insert(Bid bid) {
	...
	// if no element found, insert newNode at the key index
	if (oldNode == nullptr) {
		Node* newNode = new Node(bid, key);
		nodes.insert(nodes.begin() + key, (*newNode));
	}
	else {
		// node found, oldNode key has not been used
		if (oldNode->key == UINT_MAX) {
			...
			// if oldNode value is not UINT_MAX, it has been used, so find next open node
			while (oldNode->next != nullptr) {
				oldNode = oldNode->next;
			}
			// make oldNode point to a new Node
